...
const provider = new ethers.BrowserProvider((window as any).ethereum)
    ...
    // `startPrice` is a number in dollars; convert to wei
    const tx = await contract.createAuction(
      title,
      ethers.parseEther(startPrice.toString()) // ethers v6 parse
    )
    await tx.wait()
    ...
    const uploadPromises = files.map((file, idx) => {
      ...
      return supabase.storage
        ...
        .then(({ data, error }) => {
          ...
          const { data: publicUrlData } = supabase.storage.from("image").getPublicUrl(data.path);
